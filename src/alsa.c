...
int
alsa_object_openrec(struct audio_object *object,
                 enum audio_object_format format,
                 uint32_t rate,
                 uint8_t channels)
{
...
#undef  FORMAT
	...
	if ((err = snd_pcm_open(&self->handle, self->device ? self->device : "default", SND_PCM_STREAM_CAPTURE, 0)) < 0)
     ...
     if ((err = snd_pcm_nonblock(self->handle, 0 ))< 0)
     goto error;
...
int
alsa_object_read(struct audio_object *object,
                  void *data,
                  size_t bytes)
{
	struct alsa_object *self = to_alsa_object(object);
                   ...
                   const char *description)
{
	...
	self->is_open = 0;
	self->vtable.openrec = alsa_object_openrec;
	...
	self->vtable.read = alsa_object_read;
