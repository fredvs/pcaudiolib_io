...
int
oss_object_openrec(struct audio_object *object,
                enum audio_object_format format,
                uint32_t rate,
                uint8_t channels)
{
...
int
oss_object_read(struct audio_object *object,
                 void *data,
                 size_t bytes)
{
	struct oss_object *self = to_oss_object(object);

	if (read(self->fd, data, bytes) == -1)
		return errno;
	return 0;
}
