...
int
audio_object_openrec(struct audio_object *object,
                  enum audio_object_format format,
                  uint32_t rate,
                  uint8_t channels)
{
	if (object)
		return object->openrec(object, format, rate, channels);
	return 0;
}
...
int
audio_object_read(struct audio_object *object,
                   void *data,
                   size_t bytes)
{
	if (object)
		return object->read(object, data, bytes);
	return 0;
}
...
#ifdef NO_XAUDIO2
	if ((object = create_waveout_object(device, application_name, description)) != NULL)
		return object;
#else
