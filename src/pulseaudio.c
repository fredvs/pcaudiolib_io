...
int
pulseaudio_object_openrec(struct audio_object *object,
                       enum audio_object_format format,
                       uint32_t rate,
                       uint8_t channels)
{
...
int
pulseaudio_object_read(struct audio_object *object,
                        void *data,
                        size_t bytes)
{
	struct pulseaudio_object *self = to_pulseaudio_object(object);
                         ...
                         const char *description)
{
	...
	self->vtable.openrec = pulseaudio_object_openrec;
	...
	self->vtable.read = pulseaudio_object_read;
